          ...
          <div class="review-item acea-row gap-base">
            <div class="p-img">
              <div class="aspect-ratio p-10 border-sm border-gray-200">
                <img class="w-full h-full fit-contain" :src="customStore.carts.previewImg" alt="">
              </div>
            </div>
      ...
      <div class="review-summary shadow-lg p-xl-20 p-15">
        ...
        <el-form :model="form" ref="formRef" :rules="rules" :auto-complete="false">
          <el-form-item prop="discountCode">
            <el-input v-model="form.discountCode" placeholder="Enter your discount code" size="large" :disabled="isDiscount || isCreate">
              <template #append>
        ...
        <div class="acea-row row-right row-center text-error" v-if="isCreate && !discountDetail?.discountAmount">
        ...
        <div class="acea-row row-between-wrapper text-16 f-bold pt-20 my-20 border-t-sm border-gray-200">
          ...
          <div>
            <p :class="[discountDetail.discountAmount ? 'text-through text-gray-600' : '']">{{ currencyStore.formatToCurrency(Number(offerData.actualAmount || 0)) }}</p>
            <p v-if="discountDetail.discountAmount">{{ currencyStore.formatToCurrency(Number(offerData.actualAmount || 0) - Number(discountDetail.discountAmount || 0)) }}</p>
          </div>
        </div>
...
import {
  ...
  confirmOrderApi,
  createOrderApi,
  uploadOrderOriginImgApi
} from "~/api/modules/custom/custom";
...
const _advanceOrderNo = async () => {
  ...
  const originalImg = dataURLtoBlob(customStore.carts.originalImg); // 转回 Blob
...
const createOrderCallback = async () => {
  try {
    ...
    formData.append('actualAmount',
      String(
      Number(offerData.value?.actualAmount || 0) - Number(discountDetail.value?.discountAmount || 0)
      )
    )
    ...
    formData.append('parts', JSON.stringify(customStore.carts.parts))
    formData.append('addressId', addressId.value)
