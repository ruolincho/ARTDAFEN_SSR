<template>
  <div class="container">
    <h1 class="text-40 font-500 lg:py-50 py-30">Review Your Order</h1>
    <div class="cart-container acea-row row-between row-top">

      <div class="review-container md:pr-30">
        <div class="text-20 font-bold py-20">Order Item List</div>
          ...
          <div class="review-item acea-row gap-base" v-for="(item, index) in generateList" :key="index">
            <div class="p-img">
              <div class="aspect-square p-10 border  ">
                <img class="w-full h-full object-contain" :src="imagePrefix(item.img)" :alt="item.title">
              </div>
            </div>
            <div class="p-cont flex-1">
              ...
              <p class="text-14 font-bold">{{ item.quantity }} ×
                {{ formatToCurrency(item.unitPrice, currencyCode) }}</p>
              <p class="text-14 text-gray-500 mt-4" v-for="(v, k) in item.specs">{{ k }}: {{ v }}</p>
            </div>
          </div>
        </div>
        <div class="text-20 font-bold py-20">Shipping Address</div>
        <div class="border-b   my-20 pb-20">
          <p class="text-20 font-bold">{{ orderDetail?.address?.name }}</p>
        ...
        <div class="text-20 font-bold py-20">Payment Method</div>
        <div class="border-b   my-20 pb-20 text-20 acea-row row-between-wrapper">
          <p class="font-bold">{{ formatPayChannel(orderDetail?.order?.payChannel) }}</p>
          <p>{{ formatTimestamp(orderDetail?.order?.payTime) }}</p>
        </div>
        <div class="border-b   my-20 pb-20 text-20 acea-row row-between-wrapper">
          <p class="font-bold">Trade No</p>
      ...
      <div class="review-summary shadow-lg p-20">
        ...
        <div class="acea-row row-between-wrapper text-16 font-bold pt-20 mt-20 border-t   text-error">
