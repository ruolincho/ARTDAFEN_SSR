  ...
  <section class="filter-wrapper">
    <div class="container-fluid">
      <div class="acea-row row-between">
        <!-- 左侧筛选区域 PC端-->
        <div class="side-wrapper mr-40" v-show="appStore.device === 'pc' && groupList.length">
...
// 点击Brand类型的选项（这里需要立即更新数据）
const brandSelected = ref({} as IHome.MenuRow)
// 关闭Brand类型的选项
const closeBrandTag = () => {
...
const getProductBest = async () => {
  ...
  if (Object.keys(mutex).length) {
    initParam.categoryIds.push(mutex.config?.referenceId || mutex.id) // Mutex类型的选项
  }
...
const paramsWatch = async () => {
  ...
  // BRAND类型的值
  if (BRAND) {
    brandSelected.value = process_BRAND(BRAND)
  } else {
    brandSelected.value = {} as IHome.MenuRow
  }
...
<style scoped lang="scss">
  ...
  .expandable-wrapper {
    ...
    .expandable-main {
      ...
      .inner {
        /* 关键点：必须设置 overflow: hidden */
        /* 否则当 grid 高度缩小时，内容会溢出显示 */
        overflow: hidden;
      }
    }
