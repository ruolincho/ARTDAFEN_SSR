<template>
  ...
  <section class="sec-topic">
    <div class="container">
      <AccordionGallery :list="zoneTopic" />
    </div>
  </section>
...
import AccordionGallery from "~/components/Zone/AccordionGallery.vue";
...
const {data: zoneTopic} = await useAsyncData(
    ...
    async () => {
      ...
      const {data} = await $fetch<IResultData<IProduct.ZoneRow>>(config.public.apiBase + TRADE_MODULE + '/product/zone/topic')
      return data
    }
)

useHead(resolvePageMeta("/zone"));
