...
export function useOriginalProductJsonLd(
    ...
    if (typeof options.shippingRateUSD === 'number') {
        shippingDetailsGlobal.shippingRate = {
            "@type": "MonetaryAmount",
            "currency": "USD",
            "value": String(options.shippingRateUSD.toFixed(2))
        }
    }
    ...
    const jsonLd = computed(() => {
        ...
        const product: any = {
            ...
            "offers": {
                ...
                "priceSpecification": {
                    ...
                    "price": Number(p.retailPrice ?? 0).toFixed(2),
                    "priceType": "https://schema.org/SuggestedRetailPrice"
                },
                "url": `${siteUrl.replace(/\/+$/, '')}/original-detail/${p.id}`, // 此商品详情页的规范URL（canonical）
                "availability": p.status === '0' ? "https://schema.org/InStock" : "https://schema.org/OutOfStock",
        ...
        return [product, ...(shippingDetailsByCountry.length ? shippingDetailsByCountry : [shippingDetailsGlobal]), returnPolicy]
    })
